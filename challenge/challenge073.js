...
// 특정 키를 눌렀을 때 입력되는 문자들도 무작위로 배열되어 있습니다. 또, 같은 문자가 자판 전체에 여러 번 할당된 경우도 있고, 키 하나에 같은 문자가 여러 번 할당된 경우도 있습니다.
...
// step1. 빈 Map 객체 생성
// step2. keymap을 순회하면서 가장 작은 누름수를 기록한다. 누름수는 0이없으니까 +1 한다
// step3. step1에 2를 넣는데 기존벨류값보다 작을경우에만 교체한다.
// step 4. targets 배열을 map 돌려서, 각 단어별 최소 누름수 합 계산
// step4. result  step 4

const solution = function (keymap, targets) {
	const minKeyPressMap = new Map();
	...
	for (let i = 0; i < keymap.length; i++) {
		for (let j = 0; j < keymap[i].length; j++) {
			...
			if (
				!minKeyPressMap.has(char) ||
				minKeyPressMap.get(char) > pressCount
			) {
				minKeyPressMap.set(char, pressCount);
			}
		}
	}
	...
	const result = targets.map((word) => {
		...
		for (let char of word) {
			if (!minKeyPressMap.has(char)) {
				return -1;
			}
			total += minKeyPressMap.get(char);
		}

...
// step 1. section 배열을 순회하면서 각 인덱스 부터 칠함
// step 2. step1에 룰러 범위 더해서 어디까지 칠했는지 계산 다음 시작ㄱ점이 마지막칠한 구간 보다 클 경우만 count ++ 아니면 계속 section 순회
// 룰러 범위 -1 해야됨 시작점이 포함되어있어서 ㅇㅇ

const solution = function (n, m, section) {
	let count = 0;
	let lastPainted = 0;

	for (let i = 0; i < section.length; i++) {
		const nextStartPoint = section[i];

		if (nextStartPoint > lastPainted) {
			lastPainted = nextStartPoint + m - 1;
			count++;
		}
	}
...
console.log(solution(10, 2, [1, 4, 7]));

...
// step 1. 객체로 점수표를 만든다. 라이언형(R) 튜브형(T) 콘형(C) 프로도형(F) 제이지형(J) 무지형(M) 어피치형(A) 네오형(N) 초기점수는 0으로 세팅한다.
// step 2. survey 배열과 choices 배열을 순회하며 점수를 저장한다.
// step 2-2. 123이면 앞의 놈한태 점수 4면 0 567이면 뒤의 놈한태 점수 이걸 어떻게 구현할까가 문제 if문 사용
// step 3. 점수가 부여된 step1을 나열하는데 앞의 글자가 크거나 같으면 앞의걸 넣고 아니면 뒤에거

const solution = function (survey, choices) {
	const score = { R: 0, T: 0, C: 0, F: 0, J: 0, M: 0, A: 0, N: 0 };

	for (let i = 0; i < survey.length; i++) {
		if (choices[i] === 1) score[survey[i][0]] += 3;
		if (choices[i] === 2) score[survey[i][0]] += 2;
		if (choices[i] === 3) score[survey[i][0]] += 1;
		if (choices[i] === 4) score[survey[i][0]] += 0;
		if (choices[i] === 5) score[survey[i][1]] += 1;
		if (choices[i] === 6) score[survey[i][1]] += 2;
		if (choices[i] === 7) score[survey[i][1]] += 3;
	}
	const result = [];
	const types = ['RT', 'CF', 'JM', 'AN'];

	for (let [first, second] of types) {
		if (score[first] >= score[second]) result.push(first);

...
// step 1. p의 길이를 구한다.
// step 2. for 문을 사용하여 0부터 1씩 증가시키면서 i , p.length 만큼 t를 slice 하여 숫자로 전환 빈배열에 넣는다.
// step 3. 빈배열에 넣어둔 애들이 p 보다 작을 경우만 샌다.
...
const solution = function (t, p) {
	...
	return arr.filter((el) => el.length === pLength && Number(el) <= Number(p))
		.length;
};

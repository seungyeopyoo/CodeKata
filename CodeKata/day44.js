...
// 명함은 잊고..width height 의미있나 뒤집으면 그만인데
// 일단 각 [] 안에서 둘중에 큰애를 뽑아주고 그중 젤 큰애 , 그 다음 나머지들중에 젤 큰애 뽑으면 되는듯?
// 2차원 배열안을 정렬하고 , 앞에 중에 가장 큰거, 뒤에 중에 가장 큰거, 곱해서 넓이

let solution = function (sizes) {
	let max_width = 0;
	let max_height = 0;

	// 모든 명함을 순회하면서 회전시킨 후 최대값을 계산
	for (let i = 0; i < sizes.length; i++) {
		let [w, h] = sizes[i];
		if (w < h) [w, h] = [h, w]; // 가로와 세로를 비교하여 큰 값을 가로로 설정

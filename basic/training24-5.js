...
// step 1. arr 배열에서 중복을 제거한 arr을 가져온다 .
// step 2. if step1.length가 k와 같거나 더 크면 k만큼 push
// step 3. else면 기존 배열 집어넣고 k - step1.length 만큼 -1 push

const solution = function (arr, k) {
	const noDuplicatedNum = [...new Set(arr)];
	const result = [];

	if (noDuplicatedNum.length >= k) {
		for (let i = 0; i < k; i++) {
			result.push(noDuplicatedNum[i]);
		}
	} else {
		result.push(...noDuplicatedNum);
		for (let i = 0; i < k - noDuplicatedNum.length; i++) {
			result.push(-1);
		}
	}
